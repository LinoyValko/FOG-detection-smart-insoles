...
def _fill_one_side(df, side, step_ms=10):
    ...
    # נחזיר לטבלה רגילה ונתמזג החוצה (outer) כדי לא לאבד שורות קיימות מהצד השני
    filled_side = g.reset_index()

    out = df.merge(filled_side, on=counter_col, how='outer', suffixes=('', '_new'))

    # ניקוי עמודות כפולות: נעדיף את הערכים החדשים כשאין ישנים
...
def fill_counters_both(df, step_ms=10):
    out = df.copy()
    # קודם צד ימין, אחר כך שמאל — או בכל סדר, לא מאבדים שורות
    ...
    # מיון עדין אם יש לכם עמודות זמן; נעדיף למיין לפי המינימום הזמין
    candidate_times = []
...
def main(study, patient_num, T_num):
    ...
    # df_filled_right = fill_counter_gaps(df,'R')
    # df_filled = fill_counter_gaps(df_filled_right, 'L')
